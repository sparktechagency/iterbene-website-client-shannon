...
// Define the valid place types as a union type
type PlaceType =
  | "establishment"
  | "geocode"
  | "address"
...
export const useGoogleLocationSearch = (
  ...
  const {
    ...
    minQueryLength = 1,
    maxResults = 20,
    types = ["establishment", "geocode"] as PlaceType[],
    autoInitialize = true,
    defaultQuery = "",
  } = options;
  ...
  const initializeServices = useCallback(() => {
    try {
      ...
      // Initialize AutocompleteService (this is the recommended approach)
        ...
        autocompleteService.current!.getPlacePredictions(
          ...
          (predictions, status) => {
            ...
            if (
              ...
              const formattedPredictions: LocationPrediction[] = predictions
                .slice(0, maxResults)
                .map((prediction) => ({
