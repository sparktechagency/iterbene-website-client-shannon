...
import { useSendEventInviteMutation } from "@/redux/features/event/eventApi";
...
const EventInviteModal = ({
  ...
  //send invite
  const [inviteEvent, { isLoading: isInviteLoading }] =
  ...
  const handleInviteConnections = async () => {
    ...
    try {
      ...
      await inviteEvent(payload).unwrap();
      setInvitedPeople([]);
      setIsInviteModalOpen(false);
      setSearchTerm("");
      setPage(1);
      setAllConnections([]);
      toast.success(`Invitation sent to ${invitedPeople.length} people!`);
    } catch (error) {
          ...
          >
            {filteredConnections?.map((connection) => {
              ...
              const isAlreadyInvited =
                eventDetailsData?.pendingInterestedUsers.some(
