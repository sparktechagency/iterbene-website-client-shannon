...
const CreateGroupModal: React.FC<CreateGroupModalProps> = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      document.querySelector('.bg-content')?.classList.add('blur-sm'); // Apply blur to background content
    } else {
      document.body.style.overflow = 'auto'; // Re-enable scrolling
      document.body.style.paddingRight = '0';
      document.querySelector('.bg-content')?.classList.remove('blur-sm'); // Remove blur
    }
  ...
  const handleGroupCreate: SubmitHandler<GroupFormData> = (data) => {
    ...
    const groupData = {
      ...data,
      coverImage: groupImage || 'https://via.placeholder.com/300x150?text=Group+Cover', // Fallback to placeholder if no image is uploaded
    };
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleSubmit(handleGroupCreate)} className="space-y-4">
              ...
              <div>
                <label className="block text-gray-700 text-sm mb-1">Who can see the event?</label>
                <select
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500"
              ...
              {/* What's the interest? */}
              <div>
                <label className="block text-gray-700 text-sm mb-1">What&apos;s the interest?</label>
                <textarea
                  ...
                  {...register('interest', { required: 'Interest description is required' })}
                />
                {errors.interest && (
              ...
              {/* Add Co-leaders */}
              <div>
                <label className="block text-gray-700 text-sm mb-1">Add co-leaders</label>
                <input
                  type="text"
                ...
                <p className="text-gray-500 text-sm mt-1">
                  Co-leaders can accept or decline once you published your group.
                </p>
              </div>

              {/* Additional Settings */}
              <div>
                <label className="block text-gray-700 text-sm mb-1">Additional settings</label>
