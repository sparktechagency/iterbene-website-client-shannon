...
import React, { useState, useRef, useEffect } from "react";
import { LocateIcon, MapPin, AlertCircle } from "lucide-react";
import {
  LocationDetails,
  LocationPrediction,
  useGoogleLocationSearch,
...
const LocationSearchInput: React.FC<LocationSearchInputProps> = ({
  ...
  // Show dropdown when predictions are available
  useEffect(() => {
    if (predictions.length > 0 && inputValue.length > 0) {
      setShowDropdown(true);
    }
  ...
  // Update input when external value changes - FIXED
  useEffect(() => {
    if (value !== inputValue) {
      setInputValue(value);
    }
  }, [inputValue, value]); // Removed inputValue from dependency array
