  ...
  return (
    <section className="w-full  pt-[90px] md:pt-[130px]">
      ...
      <section className="w-full container mx-auto  grid grid-cols-1 md:grid-cols-12 gap-5">
        <div className="w-[382px] ml-[180px] hidden md:block  mx-auto col-span-full md:col-span-3 fixed left-0 top-0 space-y-5 md:space-y-5 pl-5 max-h-screen pb-5 overflow-y-scroll">
          <AuthorBio />
          <MainNavigation />
          {user ? <Invitations /> : <UpcomingEvent />}
        </div>
        <div className="w-full md:col-span-9 px-4 ml-0 md:ml-[382px]">{children}</div>
      </section>
    </section>
  );
};
